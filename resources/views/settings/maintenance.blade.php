<x-layouts.app title="Maintenance">
    <x-page-header title="Maintenance" icon="refresh" subtitle="License expiry, seat reclamation, and audit pruning windows." />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <form method="POST" action="{{ route('settings.maintenance.update') }}" class="space-y-6">
                ...
                {{-- Master switch --}}
                <x-card title="Automatic Maintenance" subtitle="A scheduled sweep that keeps licenses, seats, and logs tidy.">
                ...
                <x-card title="Housekeeping Tasks" subtitle="What the sweep does each time it runs.">
                    <div class="space-y-5">
                        <x-toggle name="expire_licenses" :checked="$v['expire_licenses'] === '1'"
                            label="Expire Past-Due Licenses"
                            description="Move active licenses whose expiry date has passed to Expired, and re-sign them." />

                        <div class="border-t border-slate-100 pt-5">
                            <x-toggle name="reap_stale_activations" :checked="$v['reap_stale_activations'] === '1'"
                                label="Reclaim Stale Activation Seats"
                                description="Delete device activations that have not checked in recently, freeing seats." />
                            <div class="mt-4 sm:max-w-xs">
                                <x-field label="Stale After (Days)" for="activation_stale_days" :error="$errors->first('activation_stale_days')"
                                    hint="An activation is stale once it goes this long without a check-in.">
                                    <x-input type="number" id="activation_stale_days" name="activation_stale_days" min="1" max="3650" value="{{ $v['activation_stale_days'] }}" />
                                </x-field>
                            </div>
                        </div>
