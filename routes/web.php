...
use App\Http\Controllers\LicenseServerController;
...
// Public node installer (curl | bash). No secret in the URL; the enroll token is passed as an arg.
Route::get('/install/node.sh', [NodeInstallerController::class, 'script'])->name('node.install');

// Authenticated control panel.
Route::middleware(['auth', 'security.policy'])->group(function () {
    Route::get('/', DashboardController::class)->name('dashboard');

    // Locations (grouping for license servers).
    Route::resource('locations', LocationController::class);

    // Catalog: products -> plans + features.
    Route::resource('products', ProductController::class);
    Route::post('products/{product}/features', [FeatureController::class, 'store'])->name('features.store');
    Route::delete('products/{product}/features/{feature}', [FeatureController::class, 'destroy'])->name('features.destroy');
    ...
    Route::post('licenses/{license}/status', [LicenseController::class, 'setStatus'])->name('licenses.setstatus');
    ...
    Route::resource('servers', LicenseServerController::class)->parameters(['servers' => 'server']);
    Route::post('servers/{server}/regenerate', [LicenseServerController::class, 'regenerate'])->name('servers.regenerate');
    ...
    // Maintenance (license expiry, seat reclamation, audit pruning + window).
